...
現代のインフラストラクチャにおいて、バッチ処理に Go を採用するメリットは大きいものです。メモリフットプリントが極めて小さく、起動はミリ秒単位。単一のバイナリ（シングルバイナリ）として動作するポータビリティの高さは、AWS ECS Tasks、GCP CloudRun、Kubernetes Jobs、サーバーレス（FaaS）のような、短寿命でクラウドネイティブな実行環境とよく噛み合います。さらに、Go で書かれた Web API のコード（DB スキーマやドメインロジック）を、そのまま 100% 共通アセットとして再利用できるという利点もあります。
