...
type HourlyHistoryToStore struct {
	Time          time.Time `gorm:"column:time;primaryKey" parquet:"time,type=INT64,logicaltype=timestamp,logicaltype.unit=millis"`
	...
	CollectedAt   time.Time `gorm:"column:collected_at" parquet:"collected_at,type=INT64,logicaltype=timestamp,logicaltype.unit=millis"`
}
