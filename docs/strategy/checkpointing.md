...
### 2.3. Lifecycle Hooks for Safety
チェックポイントの保存だけでは、再実行時の安全性は不十分である。再実行時にリソースの競合や重複処理を防ぐため、以下のライフサイクルリスナーを併用すること。
*   **BeforeStep/AfterStep:** 再実行時の初期化や、失敗時のリソースクリーンアップを保証する。
*   **BeforeChunk/AfterChunk:** トランザクション単位での冪等性担保ロジックを挿入する。
...
*   **Robustness:** `ExecutionContext` は型安全なアクセサ（`GetInt`, `GetBool`, `GetString`, `GetFloat64`）およびネスト構造（`PutNested`, `GetNested`）をサポートしている。コンポーネントはこれらを活用し、型不一致やキー衝突を未然に防ぐこと。
